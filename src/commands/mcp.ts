    ...
    .action(async (options) => {
        ...
        if (options.transport === 'stdio') {
            ...
            try {
                ...
                const datadogService = config.datadog?.api_key
                    ? new DatadogService(config)
                    : undefined;
                        ...
                        if (parameters.properties) {
                            ...
                            for (
                                ...
                                switch (prop.type) {
                                    ...
                                    case 'array': {
                                        const itemType =
                                            typeof prop.items === 'object' && prop.items
                        ...
                        const sseServer = http.createServer((req, res) => {
                            ...
                            if (
                                ...
                                const cleanup = () => {
                                    ...
                                    try {
                                        logger.debug(
                                            `Cleaning up SSE transport for session: ${transport.sessionId}`,
                                        );
                                        transport.close();
                                    } catch (error) {
                                        if (
                                ...
                                req.on('error', (error) => {
                                    if (!(error instanceof Error && error.name === 'AbortError')) {
                                        logger.error(
                                            `SSE request error for session ${transport.sessionId}:`,
                                            error,
                                        );
                                    }
                                    cleanup();
                            ...
                            if (new URL(req.url!, 'http://localhost').pathname === endpoint) {
                                try {
                                    ...
                                    if (sessionId && activeTransports[sessionId]) {
                                        ...
                                        logger.debug(
                                            `Found existing transport for session: ${sessionId}`,
                                        );
                                        ...
                                        transport = new StreamableHTTPServerTransport({
                                            ...
                                            onsessioninitialized: (sid) => {
                                                logger.info(
                                                    `New Streamable HTTP session initialized: ${sid}`,
                                                );
                                        ...
                                        transport.onclose = () => {
                                            ...
                                            if (sid && activeTransports[sid]) {
                                                logger.info(
                                                    `Streamable HTTP transport closed for session ${sid}`,
                                                );
                                        ...
                                        logger.warn(
                                            'Invalid Streamable HTTP request - missing or invalid session ID',
                                        );
                                    ...
                                    if (req.method === 'DELETE' && sessionId) {
                                        logger.info(
                                            `Received DELETE request for session: ${sessionId}`,
                                        );
                                        try {
...
function registerResources(
    ...
    server.resource(
        ...
        async (uri, { path }) => {
            try {
                logger.debug(
                    `File resource accessed with URI: ${uri.href}, extracted path: ${path}`,
                );
                ...
                try {
                    ...
                    if ('messages' in promptDef) {
                        ...
                        return {
                            content: [{
                                type: 'text',
                                text: `Invalid prompt definition for "${promptId}"`,
                            }],
                            isError: true,
                        };
                    }
                    ...
                    return {
                        content: [{
                            type: 'text',
                            text: `Error executing prompt: ${error.message}`,
                        }],
                        isError: true,
                    };
                }
