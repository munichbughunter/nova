...
const testCmd = new Command()
    ...
    .action(async () => {
        try {
            logger.passThrough(
                'log',
                colors.bold('Testing connections from current configuration...\n'),
            );
            const config = await configManager.loadConfig();
    ...
    .action(async (_, key) => {
        try {
            ...
            if (
                ...
                (parts[parts.length - 1].includes('token') ||
                    parts[parts.length - 1].includes('password'))
            ) {
                logger.passThrough('log', '***');
            } else {
