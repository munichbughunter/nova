...
const mockPageDetails = {
  ...
  body: {
    storage: {
      value: "<p>Welcome to the Team One Space!</p><h2>About Us</h2><p>This space contains documentation for Team One.</p>",
...
await snapshotTest({
  ...
  async fn() {
    ...
    console.log(colors.bold("Version:"), page.version.number ? `v${page.version.number}` : 'N/A');
    console.log(colors.bold("Created:"), `${formatLocaleDate(page.history.createdDate)} by ${page.history.createdBy.displayName}`);
    console.log(colors.bold("Last Updated:"), `${formatLocaleDate(page.history.lastUpdated)} by ${page.history.lastUpdatedBy.displayName}`);
