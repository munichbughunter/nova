...
for (const { type, description } of agentTypes) {
    ...
    if (type === 'dev') {
        subCommand
            ...
            .example(
                ...
                'nova agent dev review --path ".gitlab-ci.yml,src/file.ts"',
            )
            ...
            .option('--depth <depth:string>', 'Analysis depth (quick|normal|deep)', {
                default: 'normal',
            })
            .option(
                '--reviewer <type:string>',
                'Review perspective (junior|senior|architect|all)',
                {
                    default: 'senior',
                },
            )
        ...
        subCommand
            ...
            .example(
                'Review specific MR',
                'nova agent dev review-mr --project group/project --mr 123',
            )
            ...
            .option('-i, --interactive', 'Interactive mode with chat functionality', {
                default: false,
            })
                    ...
                    if (!options.project) {
                        try {
                            ...
                            if (repoInfo?.provider === 'gitlab') {
                                options.project = repoInfo.projectPath;
                                logger.passThrough(
                                    'log',
                            ...
                            // Fallback für den Fall, dass Detector fehlschlägt
                            try {
                                const remoteUrl = await new Deno.Command('git', {
                                    args: ['remote', 'get-url', 'origin'],
                                    stdout: 'piped',
                                }).output();
                                const remoteUrlText = new TextDecoder().decode(remoteUrl.stdout)
                                    .trim();
                                ...
                                if (match) {
                                    ...
                                    logger.passThrough(
                                        'log',
                                        colors.dim(
                                            `Using project from git remote: ${options.project}`,
                                        ),
                                    );
                                }
                            } catch {
                                if (!options.interactive) {
                                    throw new Error(
                                        'No project specified and could not determine from git remote',
                                    );
                                }
                            }
                        }
    ...
    if (type === 'qa') {
        subCommand
            ...
            .option('--depth <depth:string>', 'Test depth (quick|normal|deep)', {
                default: 'normal',
            })
            .option('--browser <browser:string>', 'Browser to use for testing', {
                default: 'chromium',
            })
...
agentCommand
    ...
    .action(() => {
        ...
        logger.passThrough(
            'log',
            `  ${colors.cyan('--post')}     - Post review comments to GitLab`,
        );
