...
await snapshotTest({
    ...
    async fn() {
        ...
        console.log(
            '  nova agent eng documentor   - Generate and manage documentation (coming soon)',
        );
        console.log(
            '  nova agent eng architect    - Architecture analysis and suggestions (coming soon)',
        );
        console.log(
            '  nova agent eng tester       - Test case generation and analysis (coming soon)',
        );
        console.log('');
...
await snapshotTest({
    ...
    async fn() {
        ...
        mockReviewFeedback.forEach((feedback, index) => {
            console.log(
                `${index + 1}. ${colors.yellow(`Line ${feedback.line}:`)} ${feedback.message}`,
            );
        });
...
await snapshotTest({
    ...
    async fn() {
        ...
        console.log(
            `Author: ${mockMergeRequest.author.name} (${mockMergeRequest.author.username})`,
        );
        console.log(
            `Source: ${mockMergeRequest.source_branch} → ${mockMergeRequest.target_branch}`,
        );
