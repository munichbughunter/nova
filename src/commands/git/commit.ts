      ...
      try {
        ...
        if (!suggestionResult.success || !suggestionResult.suggestion) {
          logger.passThrough(
            'log',
            colors.red(
              `Error generating commit suggestions: ${suggestionResult.error || 'Unknown error'}`,
            ),
          );
          Deno.exit(1);
        }
        ...
        const fullCommitMessage = commitDesc ? `${commitMessage}\n\n${commitDesc}` : commitMessage;
...
commitCommand.command('help')
  ...
  .action(() => {
    ...
    logger.passThrough('log', '  - Examples: feat(auth): add login page, fix: resolve memory leak');
    logger.passThrough('log', '\nImpact Assessment:');
    ...
    logger.passThrough('log', "  - minor: New features that don't break existing functionality");
    logger.passThrough(
      'log',
...
3. Focus on the most significant changes in the diff.`;
    ...
    const userPrompt = `Analyze these changes and generate a commit message:\n\n${truncatedDiff}`;
