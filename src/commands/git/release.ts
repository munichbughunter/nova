    ...
    .action(async () => {
        ...
        try {
            ...
            if (await checkExistingSemanticRelease()) {
                logger.passThrough(
                    'log',
                    colors.yellow('Existing semantic release configuration found.'),
                );
                ...
                logger.passThrough(
                    'log',
                    colors.green('✓ Converted existing configuration to nova.json'),
                );
            } else {
                ...
                if (tagPrefix) {
                    ...
                    logger.passThrough(
                        'log',
                        colors.dim('No tag prefix detected, using default (empty)'),
                    );
                }
                ...
                if (!force) {
                    const shouldRegenerate = await Confirm.prompt({
                        message: colors.yellow(
                            'CHANGELOG.md already exists. Regenerate from scratch?',
                        ),
                        default: false,
                    });
    ...
    .action(async () => {
        try {
            ...
            try {
                ...
                logger.passThrough(
                    'log',
                    colors.dim(`Main branches: ${config.branches.main.join(', ')}`),
                );
                logger.passThrough(
                    'log',
                ...
                logger.passThrough(
                    'log',
                    colors.dim(`Changelog enabled: ${config.changelog.enabled}`),
                );
                logger.passThrough(
                    'log',
                    colors.dim(
                        `GitLab integration: ${config.gitlab?.enabled ? 'enabled' : 'disabled'}`,
                    ),
                );
                logger.passThrough('log', colors.dim('─'.repeat(50)));
            } catch (_error) {
...
async function runRelease(options: ReleaseOptions) {
    ...
    try {
        ...
        try {
            ...
            logger.passThrough(
                'log',
                colors.dim(`Version bump type: ${versionClassification.type}`),
            );
            ...
            if (dryRun) {
                ...
                logger.passThrough(
                    'log',
                    colors.dim(
                        `- Generate changelog at ${config.changelog.path || 'CHANGELOG.md'}`,
                    ),
                );
