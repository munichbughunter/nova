    ...
    try {
      ...
      if (options.project_key) {
        ...
        try {
          ...
          selectedProject = await gitlab.getProjectDetails(mostRecent.path_with_namespace);
          logger.passThrough(
            'log',
            colors.dim(
        ...
        try {
          ...
          if (matchingProjects.length === 1) {
            ...
            selectedProject = matchingProjects.find((p) => p.path_with_namespace === selectedPath);
          }
        } catch (error) {
      ...
      try {
        ...
        const db = await DatabaseService.getInstance();
        const cachedDashboard = await db.getCachedDashboard(selectedProject.path_with_namespace);
        ...
        if (cachedDashboard && !options.refresh) {
          logger.passThrough(
            'log',
            colors.dim(`Using cached metrics from ${cachedDashboard.timestamp.toLocaleString()}`),
          );
          ...
          try {
            ...
            progress.stop();
            logger.passThrough('log', colors.green('✓ Project metrics generated successfully'));
          } catch (error) {
...
dashboardCommand.command('help')
  ...
  .action(() => {
    ...
    logger.passThrough('log', '  -d, --days <number>     Number of days to analyze [default: 30]');
    ...
    logger.passThrough('log', '  --pipeline-limit <number> Limit number of pipelines to analyze');
    logger.passThrough(
      'log',
