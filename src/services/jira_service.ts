...
export class JiraService {
            ...
            if (response.issues && response.issues.length > 0) {
                ...
                const customFields = Object.entries(sampleIssue.fields)
                    ...
                    .reduce(
                        (acc, [key, _value]) => ({
                            ...acc,
                            [key]: this.getFieldValue(sampleIssue, key),
                        }),
                        {},
                    );
                ...
                if (foundFields.length > 0) {
                    ...
                    this.logger.debug(
                        'No story points fields found. Using default:',
    ...
    public async addComment(
        ...
        try {
            ...
            const response = await this.makeRequest<{ id: string; self: string }>(
                endpoint,
                'POST',
                body,
            );
    ...
    private async getIssueStats(projectKey: string): Promise<JiraIssueStats> {
        ...
        const [assignedToMe, createdByMe, dueSoon, overdueIssues, recentUpdated] = await Promise
            .all([
                ...
                this.searchIssues(
                    `project = "${projectKey}" AND due < now() AND resolution = Unresolved`,
                ),
                this.searchIssues(
    ...
    public async getProjectMetrics(
        ...
        try {
            // Check cache first
            const cacheKey = `project_metrics_${jiraProjectKey}_${
                boardId || 'default'
            }_${Date.now()}`; // Add timestamp to force refresh
            ...
            if (!boardId && boards.length > 0) {
                ...
                const selectedBoardId = await Select.prompt({
                    ...
                    default: boards.find((b) => b.type === 'scrum')?.id.toString() ||
                        boardChoices[0].value,
                });
    ...
    formatIssueInfo(issue: JiraIssue): string {
        ...
        infoTable.push([
            '📊 Status',
            `${
                this.getStatusEmoji(issue.fields.status.statusCategory.key)
    ...
    formatIssueList(issues: JiraIssue[]): string {
        ...
        const getStatusColor = (statusCategory: string, statusName: string): string => {
            ...
            if (
                ...
                statusCategory === 'In Progress' || name.includes('progress') ||
                ...
                statusCategory === 'To Do' || name.includes('todo') || name.includes('backlog')
            ) {
        ...
        try {
            ...
            for (let i = 0; i < sprintDataArray.length; i++) {
                for (let j = i + 1; j < sprintDataArray.length; j++) {
                    ...
                    if (
                        sprint1.startDate <= sprint2.endDate && sprint2.startDate <= sprint1.endDate
                    ) {
                        // Found overlap
                        const dateRange =
                        ...
                        const existingRange = duplicateSprints.find((d) =>
                    ...
                    for (const history of issue.changelog.histories) {
                        for (const item of history.items) {
                            if (item.field === 'Sprint' && item.toString?.includes(sprintIdStr)) {
                                ...
                                if (
                                    addedDate <= planningWindowEnd || addedDate <= earlySprintWindow
                                ) {
                                    return true;
                                }
                            }
    ...
    private rateVelocity(cycleTime: number, throughput: number): string {
        const cycleTimeScore = cycleTime <= 24 * 3
            ...
            ? 1
            : 0;
        const throughputScore = throughput >= 10
            ? 3
            : throughput >= 7
            ? 2
            : throughput >= 5
            ...
            for (let i = 0; i < n; i++) {
                ...
                const completedIssues = issues.filter((issue) => {
                    const resolutionDate = issue.fields.resolutiondate
                        ? new Date(issue.fields.resolutiondate)
                        : null;
                    return resolutionDate && resolutionDate >= startDate &&
    ...
    formatProjectList(projects: JiraProject[]): string {
        ...
        const formatProject = (project: JiraProject): string => {
            ...
            // Add title as a header row instead of using .title()
            projectTable.push([
                `${theme.symbols.project} Project: ${
                    theme.emphasis(project.name)
                } (${project.key})`,
            ]);
    ...
    formatProjectDashboard(metrics: JiraProjectMetrics): string {
        ...
        sections.push(
            new Table()
                ...
                .body([
                    ...
                    [
                        'Completion Trend',
                        this.getTrendIndicator(sprints?.completionRateTrend || []),
                    ],
                ])
                .border(true)
                .toString(),
            ...
            sections.push(
                new Table()
                    ...
                    .body([
                        ...
                        [
                            'Sprint Progress',
                            `${Math.min(100, sprints.active.progress).toFixed(1)}%`,
                        ],
                        [
                            'Sprint Dates',
                        ...
                        [
                            'Daily Velocity',
                            `${sprints.active.avgDailyVelocity.toFixed(1)} points/day`,
                        ],
                    ])
                    .border(true)
                    .toString(),
        ...
        if (sprints?.history && sprints.history.length > 0) {
                ...
                new Table()
                    ...
                    .body(
                        sprints.history.map((sprint) => [
                            sprint.name || 'Unknown',
                            `${formatTimestamp(sprint.startDate)} → ${
                                formatTimestamp(sprint.endDate)
                            }`,
                            `${sprint.committedAndCompleted?.length || 0}/${
                                sprint.committedIssues || 0
                            }`,
            ...
            const spilloverTrend = this.getTrendIndicator(
                this.calculateSpilloverTrend(sprints.history),
            );
                ...
                new Table()
                    ...
                    .body([
                        ...
                        [
                            '• Average Rate',
                            `${
                                this.calculateScopeChangeRate(sprints.history).toFixed(1)
                            }% additional work`,
                        ],
                        ...
                        [
                            '• Average Rate',
                            `${
                                this.calculateSpilloverRate(sprints.history).toFixed(1)
                            }% of committed work`,
                        ],
    ...
    private calculateHealthScore(params: {
        ...
        if (pastSprints.length > 0) {
            ...
            const avgHistoricalCompletion = pastCompletionRates.length > 0
                ? pastCompletionRates.reduce((sum, rate) => sum + rate, 0) /
                    pastCompletionRates.length
                : 0;
    ...
    private getSprintMetrics(
        ...
        const velocityData = sprintData
            .map((sprint) =>
                sprint ? (typeof sprint.velocity === 'number' ? sprint.velocity : 0) : 0
            )
            .slice(0, 5)
            .reverse();
        ...
        if (latestSprint && latestSprint.state === 'active') {
            ...
            const predictabilityScore = this.calculatePredictabilityScore(
                velocityData,
                completionRates,
            );
    ...
    private calculateDeliveryScore(completionRates: number[]): string {
        const avgCompletionRate = completionRates.reduce((a, b) => a + b, 0) /
            completionRates.length;
    ...
    private async getProjectTimeline(projectKey: string): Promise<JiraTimeline> {
        ...
        const timeline: JiraTimeline = {
            ...
            updated: [{
                count: recentIssues.issues.filter((i) =>
                    new Date(i.fields.updated) >= sevenDaysAgo
                ).length,
            }],
            comments: [{
    ...
    formatSprintAnalysis(sprintData: SprintData[]): string {
        ...
        const avgCycleTime =
            sprintData.reduce((sum, sprint) => sum + sprint.averageCycleTime.mean, 0) /
            sprintData.length;
        ...
        velocityTable.push([
            'Performance Level',
            formatServiceStatus(
                this.rateVelocity(avgCycleTime, totalCompleted / sprintData.length),
            ),
        ]);
