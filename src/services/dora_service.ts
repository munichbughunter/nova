...
export class DoraService {
    ...
    private async setCachedData<T>(key: string, data: T): Promise<void> {
        ...
        try {
            ...
            this.logger.debug(
                `Cached data for ${this.extractQueryType(key)}_${key.slice(0, 50)}...`,
            );
        } catch (error) {
    ...
    private formatStandardMetrics(result: ExtendedDoraMetricsResult): string {
        ...
        headerTable.push([
            '📎 Jira Project',
            `${result.jiraProject.name} (${result.jiraProject.key})`,
        ]);
        headerTable.push([
            '🦊 GitLab Project',
        ...
        deploymentTable.push([
            'Success rate',
            `${
                ((deploymentMetrics?.deploymentsTotal || 0) /
                    (deploymentMetrics?.deploymentsTotal || 1) *
                    100).toFixed(1)
            }%`,
        ]);
        ...
        const trendData = [
            formatTrendChart(
                trends.deploymentFrequencyTrend.map((value) =>
                    ...
                    Math.min(
                        4,
                        Math.max(0, value / Math.max(...trends.deploymentFrequencyTrend) * 4),
                    )
                ),
                5,
                false,
        ...
        deploymentTable.push([
            ...
            deploymentMetrics?.environmentBreakdown?.production.environments.length > 0
                ? this.formatProductionEnvironments(
                    deploymentMetrics.environmentBreakdown.production,
                )
                : 'No deployments',
        ]);
        ...
        deploymentTable.push([
            ...
            deploymentMetrics?.environmentBreakdown?.development.environments.length > 0
                ? this.formatDevelopmentEnvironments(
                    deploymentMetrics.environmentBreakdown.development,
                )
                : 'No deployments',
        ]);
        ...
        leadTimeTable.push([
            `${theme.symbols.time} Lead Time for Changes (${timeRangeDisplay})`,
            '',
        ]);
        ...
        failureRateTable.push([
            `${theme.symbols.error} Change Failure Rate (${timeRangeDisplay})`,
            '',
        ]);
        ...
        failureRateTable.push([
            'Trend',
            formatTrendChart(trends.changeFailureRateTrend, 5, true, [
                '4w',
                '3w',
                '2w',
                '1w',
        ...
        const formatIncidentBreakdown = () => {
            ...
            return [
                `Critical: ${critical} (avg: ${
                    this.formatDurationForDisplay(avgRestoreTime * 0.5)
                })`,
        ...
        restoreTable.push([
            'Median restore time',
            this.formatDurationForDisplay(medianRestoreTime),
        ]);
    ...
    private async calculateSprintCommitment(
        ...
        try {
            ...
            if (
                !currentSprint || !currentSprint.committedIssues || !currentSprint.completedIssues
            ) {
    ...
    private async calculateSprintVelocity(
        ...
        try {
            ...
            const currentSprint = sprintData[0];
            if (
                !currentSprint || !currentSprint.averageCycleTime || !currentSprint.completedIssues
            ) {
                this.logger.error('Invalid sprint data structure:', currentSprint);
                return {
                    cycleTime: 0,
            ...
            if (throughput >= previousThroughput && cycleTime <= previousCycleTime * 1.1) {
                ...
                throughput >= previousThroughput * 0.9 && cycleTime <= previousCycleTime * 1.2
            ) {
                rating = 'good';
    ...
    private async calculateWorkTypeBreakdown(
        ...
        try {
            ...
            const categorizeIssue = (issue: JiraIssue) => {
                ...
                this.logger.debug(
                    `Categorizing issue ${issue.key} with type ${issueType} and labels ${
                        labels.join(', ')
                    }`,
                );
    ...
    private async calculateAddedTickets(
        ...
        try {
            ...
            const trend = sprintData.map((sprint) => {
                const sprintCommittedIssues = new Set(
                    sprint.committedIssues?.map((i) => i.key) || [],
                );
    ...
    private formatCustomMetrics(metrics: CustomTeamMetrics): string {
        ...
        spilloverTable.push([
            'Spillover Rate',
            `${metrics.sprintSpillover.spilloverRate.toFixed(1)}%`,
        ]);
        spilloverTable.push([
        ...
        spilloverTable.push([
            'Trend',
            formatTrendChart(metrics.sprintSpillover.trend, 5, true, [
                'S-5',
                'S-4',
                'S-3',
                'S-2',
        ...
        velocityTable.push([
            'Average Cycle Time',
            formatDuration(metrics.sprintVelocity.cycleTime),
        ]);
        velocityTable.push([
            'Median Cycle Time',
        ...
        velocityTable.push([
            'Bottlenecks',
            metrics.sprintVelocity.bottlenecks.join(', ') || 'None',
        ]);
        ...
        const technicalTrendData = [
            ...
            `Average: ${
                ...
                Math.max(...metrics.workTypeBreakdown.trend.technical).toFixed(1)
            }% | Change: ${this.getTrendArrow(metrics.workTypeBreakdown.trend.technical)}`,
        ].join('\n');
    ...
    private formatTrendAnalysis(trends: DoraTrends): string {
        ...
        deploymentTable.push([
            'Trend',
            formatTrendChart(trends.deploymentFrequencyTrend, 5, true, [
                '4w',
                '3w',
                '2w',
                '1w',
