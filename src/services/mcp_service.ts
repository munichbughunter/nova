...
export class MCPService {
        ...
        try {
            ...
            switch (toolName) {
                ...
                case 'jira_add_comment':
                    return await this.executeJiraAddComment(params, context);
                // Confluence Tools
    ...
    private async executeJiraAddComment(
        params: Record<string, unknown>,
        context: MCPToolContext,
    ): Promise<MCPToolResult> {
        try {
            ...
            const { issueKey, comment } = params as { issueKey: string; comment: string };
        ...
        try {
            ...
            if (!confluenceService) {
                return {
                    success: false,
                    error:
                        'Confluence service not available. Please configure Confluence settings.',
                };
            }
            ...
            if (!query || query.trim().length === 0) {
                query = '*';
                this.logger.info(
                    'No search query provided, using default search (*) to find recent pages',
                );
            }
    ...
    // DORA Tool Implementation
    private async executeDoraMetrics(
        params: Record<string, unknown>,
        context: MCPToolContext,
    ): Promise<MCPToolResult> {
        try {
            // Use existing DORA service if available in context
            const doraService = context.dora as DoraServiceType;
            ...
            // Extract parameters with default values to ensure they're not undefined
            ...
            const metricsResult = await doraService.getDoraMetrics(project, '', timeRange);
        ...
        try {
            ...
            if (!aiService) {
                return {
                    success: false,
                    error:
                        'AI service not available. Please provide code directly or configure AI settings.',
                };
            }
            ...
            let generatedCode;
            try {
                const response = await aiService.generateObject<CodeResponse>(
          ...
          4. Always return a valid JSON object with a 'code' property`,
                ...
                if (!response || typeof response !== 'object' || !response.code) {
                    throw new Error(
                        'Invalid response from AI service: missing or invalid code property',
                    );
                }
