...
export class GitService {
    ...
    /**
     * Get information about the git provider (GitHub/GitLab) for the current repository
     * @returns GitProviderInfo object or null if detection fails
     */
    public async getProviderInfo(): Promise<GitProviderInfo | null> {
        try {
    ...
    public async getRemoteUrl(remoteName = 'origin'): Promise<string | null> {
        try {
            const remoteUrl = await GitProviderDetector.parseGitConfig(
    ...
    /**
     * Check if the current repository is hosted on a specific provider
     * @param provider The provider to check for ('github' or 'gitlab')
     * @returns True if the repository is hosted on the specified provider
     */
    public async isProvider(provider: GitProvider): Promise<boolean> {
        try {
