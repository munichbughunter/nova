...
`;
    ...
    formatMonitorList(monitors: DatadogMonitor[]): string {
        ...
        for (const monitor of monitors) {
            sections.push(
                theme.emphasis(
                    `${formatServiceStatus(monitor.overall_state)} Monitor: ${monitor.name}`,
                ),
            );
            ...
            monitorTable.push([
                `${theme.symbols.metrics} Status`,
                formatServiceStatus(monitor.status),
            ]);
            ...
            monitorTable.push([
                `${theme.symbols.time} Created`,
                formatTimestamp(monitor.created_at),
            ]);
            monitorTable.push([
                `${theme.symbols.time} Modified`,
                formatTimestamp(monitor.modified_at),
            ]);
    ...
    formatDashboardList(dashboards: DatadogDashboard[]): string {
        ...
        for (const dashboard of dashboards) {
            ...
            if (dashboard.description) {
                dashboardTable.push([
                    `${theme.symbols.documentation} Description`,
            ...
            dashboardTable.push([
                `${theme.symbols.time} Created`,
                formatTimestamp(dashboard.created_at),
            ]);
            dashboardTable.push([
                `${theme.symbols.time} Modified`,
    ...
    formatEventList(events: DatadogEvent[]): string {
        ...
        for (const event of events) {
            sections.push(
                theme.emphasis(`${getAlertTypeEmoji(event.alert_type)} Event: ${event.title}`),
            );
    ...
    formatTeamList(teams: DatadogTeam[]): string {
        ...
        for (const team of teams) {
            ...
            if (team.links && team.links.length > 0) {
                teamTable.push([
                    `${theme.symbols.documentation} Links`,
                    team.links.length.toString(),
                ]);
            }
