...
export class AIService {
    ...
    constructor(
        ...
        if (defaultProvider === 'nova' && this.config.ai?.nova?.api_key) {
            ...
            const resourceName = azureConfig.api_url.match(/https:\/\/([^.]+)\./)?.[1] ||
                'aicp-prod-sc';
    ...
    async generateWithTools<T>(
        ...
        try {
            ...
            if (!jsonMatch) {
                this.logger.passThrough(
                    'log',
                    theme.warning('⚠️ No structured data found in response'),
                );
    ...
    public getLLMProvider(): LLMProvider {
        ...
        return {
            ...
            generateObject: async <T>(
                prompt: string,
                schema: Record<string, unknown> | z.ZodType<T>,
            ) => {
                ...
                if (tools && tools.length > 0 && this.mcpContext) {
                    ...
                    for (const toolFn of tools) {
                        try {
                            ...
                            const toolsByName = toolService.getToolsByNames(
                                [toolFn.name],
                                this.mcpContext,
                            );
    ...
    private preprocessResponse(
        ...
        for (const [key, fieldSchema] of Object.entries(schemaShape)) {
            ...
            if (value === null || value === undefined) {
                ...
                if (
                    fieldSchema instanceof z.ZodObject && typeof value === 'object' &&
                    value !== null
                ) {
    ...
    private simpleRepair(
        ...
        for (const [key, fieldSchema] of Object.entries(schemaShape)) {
            ...
            if (fieldSchema instanceof z.ZodString) {
                ...
                repaired[key] = fieldSchema.options.includes(value)
                    ? value
