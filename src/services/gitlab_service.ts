...
export class GitLabService {
    ...
    public async getProjectMetrics(
        ...
        try {
            ...
            const [project, codeQuality, mergeRequests, pipelineMetrics, teamMetrics] =
                await Promise.all(
                    [
            ...
            const metrics: GitLabProjectMetrics = {
                ...
                teamMetrics: {
                    ...
                    totalCommits: teamMetrics.topContributors.reduce(
                        (sum, c) => sum + c.commits,
                        0,
                    ),
    ...
    private async processEnvironments(
        ...
        for (const env of envs) {
            try {
                const deployments = await this.getEnvironmentDeploymentsForEnv(
                    projectPath,
                    env.name,
                );
    ...
    private async getProjectCodeQuality(fullPath: string): Promise<GitLabCodeQuality> {
        try {
            ...
            const response = await this.graphqlRequest<
                GitLabGraphQLResponse<GitLabCodeQualityResponse>
            >(
                codeQualityQuery,
                { fullPath },
            );
            ...
            const hasLoadTesting = docChecks.hasFile('k6.js') ||
                ...
                (docChecks.hasGitlabCI &&
                    jobs.some((job) => job.name.toLowerCase().includes('load-test')));
            ...
            const hasSecretScanning =
                project.securityScanners?.enabled?.includes('secret_detection') ||
    ...
    private async calculateDeploymentFrequency(
        ...
        if (oldestDeployment && newestDeployment) {
            const daysDiff = Math.max(
                1,
                Math.ceil(
                    (newestDeployment.getTime() - oldestDeployment.getTime()) /
                        (1000 * 60 * 60 * 24),
                ),
            );
        ...
        return this.getProjectCodeQuality(fullPath).catch(async (error) => {
            ...
            try {
                ...
                const docChecks = await this.checkDocumentation(fullPath, 'main')
                    ...
                    .catch(() => ({
                        project: {
                            repository: { tree: { blobs: { nodes: [] }, trees: { nodes: [] } } },
                        },
                    }));
                ...
                this.logger.debug(
                    `Error checking for test files: ${checkError}. Assuming no tests.`,
                );
            }
    ...
    private async checkDocumentation(
      ...
      `;
            ...
            const response = await this.graphqlRequest<
                GitLabGraphQLResponse<DocumentationResponse>
            >(
                docQuery,
                { fullPath },
            );
            ...
            return {
                ...
                hasFile,
                hasTests: project.pipelines?.nodes?.some((
                    p: { jobs: { nodes: Array<{ name: string }> } },
    ...
    private async getTeamMetrics(
        ...
        project(fullPath: $fullPath) {
          ...
          mergeRequests(last: $mrLimit) {
            ...
            const response = await this.graphqlRequest<
                GitLabGraphQLResponse<GitLabTeamMetricsResponse>
            >(
                teamMetricsQuery,
            ...
            const reviewParticipation = totalTeamMembers > 0
                ? (reviewingMembers / totalTeamMembers)
                : 0;
  ...
  project(fullPath: $fullPath) {
    ...
    totalPipelines: pipelines {
      count
            ...
            const response = await this.graphqlRequest<
                GitLabGraphQLResponse<GitLabPipelineResponse>
            >(
                pipelineQuery,
                { fullPath, limit },
            );
            ...
            const successRate = totalPipelines > 0
                ? (succeededPipelines / totalPipelines) * 100
                : 0;
    ...
    private async getEnvironmentDeploymentsForEnv(
        ...
        try {
            ...
            this.logger.error(
                `Error getting deployments for environment ${environmentName}:`,
                error,
            );
            return [];
        }
    ...
    public formatProjectInfo(project: ProjectSchema): string {
        ...
        if (project.snippets_enabled !== undefined) {
            detailsTable.push([
                'Snippets',
                project.snippets_enabled ? 'Enabled ✅' : 'Disabled ❌',
            ]);
        }
    ...
    public formatProjectMetrics(metrics: GitLabProjectMetrics): string {
        ...
        const result: string[] = [];
        result.push(
            colors.bold.blue(`📊 Project Dashboard: ${project?.name || 'Unknown'} ${status}`),
        );
        ...
        const teamTable = new Table()
            ...
            .body([
                ...
                [
                    ...
                    `${
                        ((metrics.teamMetrics?.averageTimeToFirstReview || 0) / 24).toFixed(1)
                    } days`,
                    '',
                ],
                [
    ...
    public async getCurrentMergeRequest(): Promise<GitLabMergeRequest | null> {
        try {
            ...
            const projectPath = repoInfo.projectPath;
    ...
    private async getEnvironments(projectPath: string): Promise<GitLabEnvironment[]> {
        try {
            ...
            const response = await this.graphqlRequest<
                GitLabGraphQLResponse<GitLabEnvironmentsResponse>
            >(
                environmentsQuery,
                { fullPath: projectPath },
            );
    ...
    public async searchProjects(search: string): Promise<ProjectSchema[]> {
      ...
      `;
            ...
            const response = await this.graphqlRequest<
                GitLabGraphQLResponse<SearchProjectsResponse>
            >(
                searchProjectsQuery,
                { search },
            );
    ...
    public async getRawFile(
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.logger.error(
                    `Error fetching raw file: ${response.status} ${response.statusText}`,
                );
    ...
    public async getProjectActivityLightRest(projectId: string | number): Promise<{
        ...
        try {
            ...
            const created_at = typeof commitDate === 'string'
                ? commitDate
                : new Date().toISOString();
    ...
    private isGitLabEnvironmentWithDeployment(
        env: unknown,
    ): env is GitLabEnvironmentWithDeployment {
        ...
        if (
            !(typeof deployment === 'object' && deployment !== null && 'created_at' in deployment)
        ) {
            return false;
        }
