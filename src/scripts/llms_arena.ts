...
const mockAIResponseHandler = {
  handleResponse: async (response: any, schema: z.ZodType<any>) => {
    try {
      ...
      throw new Error(
        `Failed to handle AI response: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  },
};
...
async function updateDocsFile(partialResults: PartialResults): Promise<void> {
  try {
      ...
      for (const modelResult of partialResults.standardTests) {
        ...
        if (test) {
          ...
          if (test.result.success) {
            ...
            notes = resultString.length > 50 ? resultString.substring(0, 47) + '...' : resultString;
          } else {
    ...
    for (const modelResult of partialResults.standardTests) {
      ...
      if (errorTests.length > 0) {
        ...
        for (const test of errorTests) {
          ...
          const shortError = errorMsg.length > 30 ? errorMsg.substring(0, 27) + '...' : errorMsg;
    ...
    for (const modelResult of partialResults.standardTests) {
      const successRate = modelResult.results.filter((r) => r.result.success).length /
        modelResult.results.length;
      const avgTime = modelResult.results.reduce((sum, r) => sum + r.result.metadata.duration, 0) /
        modelResult.results.length;
