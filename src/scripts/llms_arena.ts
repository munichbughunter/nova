...
const TEST_CASES = [
  ...
  },
  {
    ...
    description: 'Tests the model\'s ability to follow a system prompt and generate specific information about TypeScript benefits.',
    test: async (model: string) => {
      const response = await generateText({
        model: ollama(model),
        messages: [
  ...
  {
    name: 'Tool Usage',
    description: 'Tests the model\'s ability to use the weather tool to retrieve information about San Francisco.',
    test: async (model: string) => {
      try {
        const response = await generateText({
          model: ollama(model),
        ...
        if (response.text && typeof response.text === 'string') {
          ...
          if (!response.text.includes('temperature') && !response.text.includes('forecast')) {
  ...
  },
  {
    ...
    description: 'Tests the model\'s ability to classify a movie plot into a genre from a predefined list.',
    test: async (model: string) => {
  ...
  },
  {
    ...
    description: 'Tests the model\'s ability to generate dates in a specific format for historical events.',
    test: async (model: string) => {
      const response = await generateObject({
        model: ollama(model),
      ...
      try {
        const response = await generateText({
          ...
          messages: [
            { 
              role: 'system', 
              content: 'You are solving math problems. Reason step by step. Use the calculator when necessary. The calculator can only do simple additions, subtractions, multiplications, and divisions. When you give the final answer, provide an explanation for how you got it.'
            },
            { 
              role: 'user', 
              content: 'A taxi driver earns $9461 per 1-hour work. If he works 12 hours a day and in 1 hour he uses 14-liters petrol with price $134 for 1-liter. How much money does he earn in one day?'
            }
          ],
          tools: [calculate],
          maxSteps: 5,
  ...
  {
    name: 'Code Review',
    description: 'Tests the model\'s ability to review code with specific formatting requirements.',
    test: async (model: string) => {
...
Provide clear, actionable feedback that helps improve the code.`,
  ...
  },
  {
    ...
    description: 'Tests the model\'s ability to generate browser actions for a search interface.',
    test: async (model: string) => {
...
const mockAIResponseHandler = {
  handleResponse: async (response: any, schema: z.ZodType<any>) => {
    try {
      ...
      throw new Error(`Failed to handle AI response: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
};
...
function printReport(partialResults: PartialResults) {
  ...
  const performanceTable = new Table()
    .header(['Model', 'Success Rate', 'Avg Time (ms)', 'Best Case', 'Worst Case', 'Common Errors'])
    .border(true);
  ...
  for (const modelResult of partialResults.standardTests) {
    ...
    const avgTime = Math.round(modelResult.results.reduce((sum, r) => sum + r.result.metadata.duration, 0) / modelResult.results.length);
...
async function updateDocsFile(partialResults: PartialResults): Promise<void> {
  try {
    ...
    docsContent = docsContent.replace(/\*\*Last Updated:\*\* .*/, 
      `**Last Updated:** ${currentDate}`);
      
    // Replace environment info placeholder - only replace once
    const envInfo = `MacBook ${Deno.build.os} ${Deno.build.target}`;
    docsContent = docsContent.replace(/\*\*Test Environment:\*\* .*/, 
      `**Test Environment:** ${envInfo}`);
    
    // Update performance overview table
    let overviewTable = '| Model | Success Rate | Avg. Response Time | Best Performing Test | Worst Performing Test |\n';
    overviewTable += '|-------|--------------|-------------------|----------------------|----------------------|\n';
    ...
    for (const modelResult of partialResults.standardTests) {
      ...
      const bestTest = testTimes.length > 0 
        ? testTimes.reduce((best, current) => current.time < best.time ? current : best).name
        : 'N/A';
        
      const worstTest = testTimes.length > 0
        ? testTimes.reduce((worst, current) => current.time > worst.time ? current : worst).name
        : 'N/A';
      
      overviewTable += `| ${modelResult.model} | ${successRate}% | ${avgTime}ms | ${bestTest} | ${worstTest} |\n`;
    }
    
    // Find the Performance Overview section and replace the table once
    const overviewSection = docsContent.match(/## Performance Overview\s*\n\s*\|.*\n\|.*\n([^#]*)/);
    if (overviewSection) {
      docsContent = docsContent.replace(overviewSection[0], 
    ...
    if (testCaseMatch) {
      docsContent = docsContent.replace(testCaseMatch[0], `\n## Test Case Results\n\n${testCaseResults}`);
    } else {
    ...
    for (const modelResult of partialResults.standardTests) {
      ...
      if (errorTests.length > 0) {
        ...
        for (const [error, tests] of errorGroups.entries()) {
          ...
          errorsTable += `| ${modelResult.model} | ${error} | ${tests.join(', ')} | ${solution} |\n`;
        }
      }
    }
    ...
    if (docsContent.match(errorsSection)) {
      docsContent = docsContent.replace(errorsSection, 
        `\n## Common Errors\n\n${errorsTable}\n`);
    }
    ...
    for (const agentResult of partialResults.agentTests) {
      ...
      const notes = agentResult.error || (agentResult.results 
        ? `${agentResult.results.filter(r => r.success).length}/${agentResult.results.length} tests passed`
        : '');
        
      agentTable += `| ${agentResult.model} | ${success} | ${agentResult.time} | ${notes} |\n`;
    }
    ...
    if (docsContent.match(matrixSection)) {
      docsContent = docsContent.replace(matrixSection, 
        `\n## Test Support Matrix\n\n${supportTable}\n`);
    }
    ...
    for (const modelResult of partialResults.standardTests) {
      const successRate = modelResult.results.filter(r => r.result.success).length / modelResult.results.length;
    ...
    const bestForStructuredData = findBestModelForTest('Generate Object with Array', partialResults);
    ...
    for (const modelResult of partialResults.standardTests) {
      const successRate = modelResult.results.filter(r => r.result.success).length / modelResult.results.length;
      const avgTime = modelResult.results.reduce((sum, r) => sum + r.result.metadata.duration, 0) / modelResult.results.length;
    ...
    if (docsContent.match(recommendationsSection)) {
      docsContent = docsContent.replace(recommendationsSection, 
        `\n## Recommendations\n\n${recommendations}\n`);
    }
    ...
    historyTable += `| ${currentDate} | ${bestOverallModel} | ${(bestOverallRate * 100).toFixed(1)}% | Updated test metrics |${existingHistory}`;
    
    // Replace test history section
    if (historyMatch) {
      docsContent = docsContent.replace(historyMatch[0], `\n## Test History\n\n${historyTable}\n`);
    } else {
...
async function createInitialReport(): Promise<void> {
  try {
    ...
    logger.info('Initial test report created. You can now run tests or update with actual results.');
  } catch (error) {
...
async function main() {
  try {
    ...
    if (Deno.args.includes('--update')) {
      const partialResults = await loadPartialResults();
      if (!partialResults) {
        logger.error('No test results found. Run the tests first to generate results or use --init-report to create an initial report.');
        return;
      }
      
      // Allow updating even if the tests aren't complete
      if (partialResults.standardTests.length > 0) {
        logger.info(`Updating documentation with ${partialResults.standardTests.length} model test results...`);
    ...
    if (partialResults.phase === 'standard') {
      for (const model of MODELS_TO_TEST) {
        if (partialResults.lastCompletedModel && 
            MODELS_TO_TEST.indexOf(partialResults.lastCompletedModel) >= MODELS_TO_TEST.indexOf(model)) {
