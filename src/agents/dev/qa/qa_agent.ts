...
export class QAAgent extends BaseDevAgent {
    ...
    constructor(context: QAAgentContext, options: QATesterOptions = {}) {
        ...
        if (this.options.stagehandModel) {
            ...
            this.context.config.ai?.default_provider === 'ollama' &&
            this.context.config.ai.ollama?.model
        ) {
  ...
  nova agent eng qa-tester generate-test --url https://example.com
        ...
        try {
            ...
            for (const [index, step] of testPlan.steps.entries()) {
                this.logger.passThrough(
                    'log',
                    `\n${
                        colors.blue(`Step ${index + 1}/${testPlan.steps.length}:`)
                    } ${step.description}`,
                );

                // Process the test step
                const stepResult = await this.processTestStep(
                    step.description,
                    step.selector,
                    step.value,
                );
    ...
    private async completeTestSession(): Promise<void> {
        ...
        this.logger.passThrough(
            'log',
            `Status: ${
                this.currentSession.success ? colors.green('Passed') : colors.red('Failed')
            }`,
        );
        ...
        try {
            ...
            if (this.stagehand.generateCode) {
                try {
                    ...
                    this.logger.debug(
                        'Stagehand code generation failed, falling back to AI:',
                        codeGenError,
                    );
                }
            }
            ...
            if (instruction.toLowerCase().includes('navigate')) {
                ...
                instruction.toLowerCase().includes('assert') ||
                instruction.toLowerCase().includes('check')
            ) {
                if (selector) {
