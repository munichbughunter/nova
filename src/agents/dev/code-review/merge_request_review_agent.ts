...
Examples:
  ...
  public async selectProjectAndMR(): Promise<MergeRequestSelection | null> {
    ...
    try {
      ...
      if (!projectPath) {
        try {
          ...
          if (match) {
            ...
            this.context.logger.passThrough(
              'log',
              theme.dim(`Using project from git remote: ${projectPath}`),
            );
          }
        } catch (error) {
      ...
      this.context.logger.passThrough(
        'log',
        theme.info(`Found ${openMRs.length} open merge requests out of ${mrs.length} total`),
      );
      ...
      this.context.logger.error(
        `Error selecting project and MR: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    }
  }
  ...
  private async createMergeRequest(projectPath: string): Promise<MergeRequestSelection | null> {
    try {
      ...
      this.context.logger.error(
        theme.error(
          `Error creating merge request: ${error instanceof Error ? error.message : String(error)}`,
        ),
      );
  ...
  private async reviewMergeRequestImproved(
    ...
    try {
            ...
            try {
              ...
              if (userInput.toLowerCase().startsWith('submit')) {
                ...
                if (confirmation.toLowerCase() === 'yes') {
                  ...
                  this.context.logger.passThrough(
                    'log',
                    theme.success(
                      `Review ${isDraft ? 'draft ' : ''}submitted to GitLab MR !${mr.iid}`,
                    ),
                  );
                  inInteractiveMode = false;
                }
              ...
              if (userInput.toLowerCase().startsWith('explore ')) {
                ...
                if (fileAnalysis) {
                  ...
                  if (fileAnalysis.issues.length > 0) {
                    ...
                    fileAnalysis.issues.forEach((issue, index) => {
                      ...
                      if (issue.suggestion) {
                        this.context.logger.passThrough(
                          'log',
                          `   ${theme.dim('Suggestion:')} ${issue.suggestion}`,
                        );
                      }
                    });
                  }
                  ...
                  this.context.logger.passThrough(
                    'log',
                    theme.warning(`File ${fileName} not found in the analysis results.`),
                  );
                }
                continue;
              }
              ...
              if (userInput.toLowerCase().startsWith('issues ')) {
                ...
                if (fileAnalysis && fileAnalysis.issues.length > 0) {
                  ...
                  if (selectedIssues.length > 0) {
                    this.context.logger.passThrough(
                      'log',
                      theme.success(`Selected ${selectedIssues.length} issues to address`),
                    );
                    ...
                    if (generateFixes.toLowerCase() === 'yes') {
                      ...
                      if (fileChange && fileChange.diff) {
                        ...
                        this.context.logger.passThrough(
                          'log',
                          theme.warning(`Could not find diff information for ${fileName}`),
                        );
                      }
                    }
                  } else {
      ...
      this.context.logger.error(
        `Error reviewing merge request: ${error instanceof Error ? error.message : String(error)}`,
      );
      return {
        success: false,
        ...
        if (hasLineInfo) {
          ...
          data.reviews.forEach((review) => {
            review.issues.forEach((issue) => {
              ...
              output.push(
                `| ${review.path} | ${severity} | ${issue.message} | ${issue.line || 'N/A'} |\n`,
              );
            });
          });
  ...
  private displayReviewSummary(
    ...
    try {
      ...
      if (session.mr) {
        summaryTable.push([
          `${theme.symbols.info || 'ℹ️'} Reviewing MR !${session.mr.iid}: ${session.mr.title}`,
        ]);
