...
import { FileAnalysis, ReviewSession, ReviewSynthesis } from './schemas.ts';
  ...
  nova agent eng review-mr --depth=deep --post
    ...
    override execute(_command: string, _args: string[]): Promise<AgentResponse> {
        try {
            ...
            formatError(
                `Error in MergeRequestReviewAgent: ${
                    error instanceof Error ? error.message : String(error)
                }`,
            );
            return Promise.resolve({
                success: false,
                message: `Error in MergeRequestReviewAgent: ${
                    error instanceof Error ? error.message : String(error)
                }`,
            });
        }
    ...
    private async analyzeFileDiff(
        ...
        try {
            ...
            this.context.logger.passThrough(
                'log',
                theme.dim('🧐 Reading and understanding the code...'),
            );
            ...
            return {
                ...
                issues: analysis.issues.map((issue) => ({
                    ...
                    line: issue.line
                        ? this.resolveLineNumberInDiff(diffLines, issue.line)
                        : undefined,
                    column: issue.column,
                    code: issue.code,
                })),
    ...
    private async reviewMergeRequest(options: BaseEngineeringOptions): Promise<AgentResponse> {
            ...
            try {
                ...
                if (mr.state !== 'opened') {
                    return {
                        success: false,
                        message:
                            `Merge request !${mr.iid} is not open (current state: ${mr.state}). This agent only reviews open merge requests.`,
                    };
                }
            } catch (error) {
                return {
        ...
        if (!mr.changes || mr.changes.length === 0) {
            try {
                ...
                return {
                    success: false,
                    message: `Failed to get merge request changes: ${
                        error instanceof Error ? error.message : String(error)
                    }`,
                };
            }
        ...
        for (const change of mr.changes || []) {
            ...
            if (analysis) {
                ...
                this.context.logger.passThrough(
                    'log',
                    theme.success(`✓ Analysis completed: ${analysis.issues.length} issue(s) found`),
                );
            }
        }
     ...
     */
    private async promptForIssueSelection(
        issues: FileAnalysis['issues'],
    ): Promise<FileAnalysis['issues']> {
        if (issues.length === 0) {
            return [];
        }
    ...
    private async generateBatchFixes(
        ...
        this.context.logger.passThrough(
            'log',
            theme.dim(`Processing ${uniqueIssues.length} unique issues...`),
        );
        ...
        for (const issue of uniqueIssues) {
            ...
            if (fixSuggestion && this.isFixDifferentFromOriginal(codeSnippet, fixSuggestion)) {
                ...
                for (const similarIssue of issues) {
                    if (
                        similarIssue !== issue && this.areIssuesSimilar(issue, similarIssue) &&
                        !fixesMap.has(similarIssue)
                    ) {
                ...
                this.context.logger.passThrough(
                    'log',
                    theme.warning(
                        `⚠️ Generated fix for "${issue.message}" doesn't significantly differ from original - skipping`,
                    ),
                );
            }
            ...
            this.context.logger.passThrough(
                'log',
                `Generating fixes (${count}/${uniqueIssues.length})`,
            );
        }
    ...
    private areIssuesSimilar(
        issue1: FileAnalysis['issues'][0],
        issue2: FileAnalysis['issues'][0],
    ): boolean {
        return (
        ...
        try {
            ...
            if (this.options.project && this.options.mergeRequest) {
                ...
                if (mr) {
                    ...
                    this.context.logger.passThrough(
                        'log',
                        theme.success(`Using specified merge request: !${mr.iid} ${mr.title}`),
                    );
                    return {
            ...
            if (!projectPath) {
                try {
                    const repoInfo = await GitProviderDetector.getCurrentRepositoryInfo();
                    ...
                    this.context.logger.debug(
                        `Failed to get project from git remote: ${
                            error instanceof Error ? error.message : String(error)
                        }`,
                    );
                }
            ...
            if (!projectPath) {
                ...
                if (!projectPath) {
                    this.context.logger.passThrough(
                        'log',
                        theme.warning('No project path provided.'),
                    );
                    return null;
                }
            }
            ...
            this.context.logger.passThrough(
                'log',
                theme.info(
                    `Found ${openMRs.length} open merge requests out of ${mrs.length} total`,
                ),
            );
            ...
            if (!openMRs || openMRs.length === 0) {
                this.context.logger.passThrough(
                    'log',
                    theme.warning('No open merge requests found for this project.'),
                );
                return null;
            }
            ...
            this.context.logger.error(
                `Error selecting project and MR: ${
                    error instanceof Error ? error.message : String(error)
                }`,
            );
            return null;
    ...
    private resolveLineNumberInDiff(
        diffLines: string[],
        lineInDiff: number | string | undefined,
    ): number | undefined {
    ...
    private async createMergeRequest(projectPath: string): Promise<MergeRequestSelection | null> {
        try {
            ...
            this.context.logger.passThrough(
                'log',
                theme.success(`✓ Merge request !${mr.iid} created: ${mr.web_url}`),
            );
    ...
    private async reviewMergeRequestImproved(
        ...
        try {
            ...
            if (!mr.changes || mr.changes.length === 0) {
                this.context.logger.passThrough(
                    'log',
                    theme.dim('Fetching merge request changes...'),
                );
            ...
            if (mr.changes?.length) {
                for (const change of mr.changes) {
                    if (change.deleted_file) {
                        this.context.logger.passThrough(
                            'log',
                            theme.dim(`📄 Skipping deleted file: ${change.new_path}`),
                        );
                        continue;
                    }
            ...
            if (
                session.overallReview.fileAnalyses && session.overallReview.fileAnalyses.length > 0
            ) {
                this.context.logger.passThrough(
                    'log',
                ...
                session.overallReview.score = session.overallReview.fileAnalyses.reduce((acc, r) =>
                    acc + r.score, 0) /
                    ...
                    while (inInteractiveMode) {
                        try {
                            ...
                            if (
                                ...
                                this.context.logger.passThrough(
                                    'log',
                                    theme.info('Exiting interactive mode...'),
                                );
                                continue;
                            }
                            ...
                            if (userInput.toLowerCase() === 'help') {
                                ...
                                this.context.logger.passThrough(
                                    'log',
                                    '• explore <filename> - See details for a specific file',
                                );
                                this.context.logger.passThrough(
                                    'log',
                                    '• issues <filename> - Select and address issues in a file',
                                );
                                this.context.logger.passThrough(
                                    'log',
                                    '• comment <filename> - Add a comment to a file',
                                );
                                this.context.logger.passThrough(
                                    'log',
                                    '• suggest <filename> - Get improvement suggestions for a file',
                                );
                                this.context.logger.passThrough(
                                    'log',
                                    '• fix <filename> <issue_number> - Generate a code fix for an issue',
                                );
                                this.context.logger.passThrough(
                                    'log',
                                    '• summary - Show the review summary again',
                                );
                                this.context.logger.passThrough(
                                    'log',
                                    '• submit [draft] - Submit the review to GitLab',
                                );
                                this.context.logger.passThrough(
                                    'log',
                            ...
                            if (userInput.toLowerCase().startsWith('explore ')) {
                                ...
                                const fileAnalysis = session.overallReview.fileAnalyses.find((a) =>
                                    a.path === fileName
                                );
                                ...
                                if (fileAnalysis) {
                                    ...
                                    this.context.logger.passThrough(
                                        'log',
                                        `Score: ${fileAnalysis.score.toFixed(1)}/10`,
                                    );
                                    this.context.logger.passThrough(
                                        'log',
                                        `Issues: ${fileAnalysis.issues.length}`,
                                    );
                                    ...
                                    this.context.logger.passThrough(
                                        'log',
                                        theme.warning(
                                            `File ${fileName} not found in the analysis results.`,
                                        ),
                                    );
                                }
                                continue;
                            ...
                            if (userInput.toLowerCase().startsWith('issues ')) {
                                ...
                                if (fileAnalysis && fileAnalysis.issues.length > 0) {
                                    ...
                                    const selectedIssues = await this.promptForIssueSelection(
                                        fileAnalysis.issues,
                                    );
                                    ...
                                    if (selectedIssues.length > 0) {
                                        this.context.logger.passThrough(
                                            'log',
                                            theme.success(
                                                `Selected ${selectedIssues.length} issues to address`,
                                            ),
                                        );
                                        ...
                                        if (generateFixes.toLowerCase() === 'yes') {
                                            ...
                                            const fileChange = mr.changes?.find((c) =>
                                                c.new_path === fileName
                                            );
                                                ...
                                                for (const issue of selectedIssues) {
                                                    ...
                                                    try {
                                                        ...
                                                        const codeSnippet = this
                                                            .extractSnippetFromDiff(
                                                                fileChange.diff,
                                                                issue.line,
                                                            );

                                                        const fixPrompt =
                                                            `You are a code fixing expert. Generate a code fix for the following issue in file ${fileName}:
                            ...
                            Provide only the fixed code without explanations. Make minimal changes to address the issue.`;
                                                        ...
                                                        const saveFix = await Input.prompt({
                                                            message:
                                                                'Save this fix as a comment on the MR? (yes/no)',
                                                            default: 'no',
                                                        });
                                                        ...
                                                        if (saveFix.toLowerCase() === 'yes') {
                                                            session.comments.push({
                                                                ...
                                                                line: issue.line !== undefined
                                                                    ? (typeof issue.line ===
                                                                            'string'
                                                                        ? parseInt(issue.line, 10)
                                                                        : issue.line)
                                                                    : null,
                                                                content:
                                                                    `## Proposed Fix for: ${issue.message}\n\n\`\`\`${
                                                                        fileName.split('.').pop() ||
                                                                        ''
                                                                    }\n${fixResponse}\n\`\`\``,
                                                                isDraft: false,
                                                ...
                                                this.context.logger.passThrough(
                                                    'log',
                                                    theme.warning(
                                                        `Could not find diff information for ${fileName}`,
                                                    ),
                                                );
                                            }
                                        }
                                        ...
                                        this.context.logger.passThrough(
                                            'log',
                                            theme.info('No issues selected'),
                                        );
                                    }
                                } else {
                                    this.context.logger.passThrough(
                                        'log',
                                        theme.warning(`No issues found in ${fileName}`),
                                    );
                                }
                                continue;
                            }
                            ...
                            if (userInput.toLowerCase().startsWith('comment ')) {
                                ...
                                if (fileAnalysis) {
                                    ...
                                    if (commentText.trim()) {
                                        ...
                                        this.context.logger.passThrough(
                                            'log',
                                            theme.success(`Comment added to ${fileName}`),
                                        );
                                    }
                                } else {
                                    this.context.logger.passThrough(
                                    ...
                                    if (fileChange && fileChange.diff) {
                                        try {
                                            ...
                                            const suggestionPrompt =
                                                `You are a code review expert. Analyze the following changes in ${fileName} and provide 3-5 specific, actionable suggestions to improve the code quality, readability, performance, or security.
                                            ...
                                            const saveSuggestions = await Input.prompt({
                                                message:
                                                    'Save these suggestions as comments on the MR? (yes/no)',
                                                default: 'no',
                                            });
                                            ...
                                            if (saveSuggestions.toLowerCase() === 'yes') {
                                                ...
                                                this.context.logger.passThrough(
                                                    'log',
                                                    theme.success('Suggestions saved as a comment'),
                                                );
                                            }
                                        } catch (error) {
                                            this.context.logger.error(
                            ...
                            if (userInput.toLowerCase().startsWith('fix ')) {
                                ...
                                if (parts.length < 2) {
                                    this.context.logger.passThrough(
                                        'log',
                                        theme.warning('Usage: fix <filename> <issue_number>'),
                                    );
                                    continue;
                                }
                                ...
                                if (fileAnalysis) {
                                    if (
                                        isNaN(issueNumber) || issueNumber <= 0 ||
                                        issueNumber > fileAnalysis.issues.length
                                    ) {
                                        this.context.logger.passThrough(
                                            'log',
                                            theme.warning(
                                                `Invalid issue number. Please specify a number between 1 and ${fileAnalysis.issues.length}`,
                                            ),
                                        );
                                        continue;
                                    }
                                    ...
                                    if (fileChange && fileChange.diff) {
                                        try {
                                            // Extract code context around the issue
                                            const codeSnippet = this.extractSnippetFromDiff(
                                                fileChange.diff,
                                                issue.line,
                                            );
                                        ...
                                        Provide only the fixed code without explanations. Make minimal changes to address the issue.`;
                                            ...
                                            if (saveFix.toLowerCase() === 'yes') {
                                                ...
                                                this.context.logger.passThrough(
                                                    'log',
                                                    theme.success('Fix saved as a comment'),
                                                );
                                            }
                                        } catch (error) {
                                            this.context.logger.error(
            ...
            this.context.logger.error(
                `Error reviewing merge request: ${
                    error instanceof Error ? error.message : String(error)
                }`,
            );
            return {
                success: false,
                message: `Error reviewing merge request: ${
                    error instanceof Error ? error.message : String(error)
                }`,
            };
        }
    ...
    private formatReviewOutput(data: { reviews: FileAnalysis[] }): string {
        ...
        try {
            ...
            const avgScore = data.reviews.reduce((sum, review) => sum + review.score, 0) /
                totalFiles;
            let scoreEmoji = '🟢';
            ...
            if (totalCount > 0) {
                ...
                for (const review of data.reviews) {
                    ...
                    const mediumIssues = review.issues.filter((issue) =>
                        issue.severity === 'medium'
                    );
            ...
            const allLearningOpportunities = data.reviews.flatMap((a) =>
    ...
    private displayReviewSummary(
        ...
        if (
            !session || !session.overallReview || !session.overallReview.fileAnalyses ||
            session.overallReview.fileAnalyses.length === 0
        ) {
            this.context.logger.debug(
                'Creating a default session for statistics as none was provided',
            );
            ...
            if (synthesis) {
                ...
                defaultSession.overallReview.suggestions = synthesis.actionItems.map((item) =>
                    item.description
                );
            }
        ...
        try {
            ...
            if (!synthesis) {
                this.context.logger.passThrough(
                    'log',
                    theme.error('No review synthesis available'),
                );
                return;
            }
            ...
            if (session.mr) {
                summaryTable.push([
                    `${
                        theme.symbols.info || 'ℹ️'
                    } Reviewing MR !${session.mr.iid}: ${session.mr.title}`,
                ]);
            ...
            if (
                options?.verbose && synthesis.learningOpportunities &&
                synthesis.learningOpportunities.length > 0
            ) {
    ...
    private displayReviewStatistics(session: ReviewSession): void {
        try {
            ...
            const issuePercentage = totalFiles > 0
                ? Math.round((filesWithIssues / totalFiles) * 100)
                : 0;
            const highPercentage = totalIssues > 0
                ? Math.round((highIssues / totalIssues) * 100)
                : 0;
            const mediumPercentage = totalIssues > 0
                ? Math.round((mediumIssues / totalIssues) * 100)
                : 0;
            ...
            const stats = [
                ...
                `Total issues: ${
                    session.overallReview.fileAnalyses?.reduce(
