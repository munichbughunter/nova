    ...
    constructor(context: ReviewAgentContext, options: BaseEngineeringOptions) {
        ...
        this.aiService = new AIService(this.context.config, {
            model: options.aiModel ||
                (options.analysisDepth === 'deep' ? 'gpt-4' : 'gpt-3.5-turbo'),
        ...
        try {
            ...
            for (let i = 0; i < _args.length; i++) {
                ...
                if (arg === '--path' && i + 1 < _args.length) {
                    ...
                    if (
                        !['junior', 'senior', 'architect', 'all'].includes(
                            options.reviewer as string,
                        )
                    ) {
                        throw new Error(
                ...
                try {
                    ...
                    if (changedFiles.length > 0) {
                        this.context.logger.passThrough(
                            'log',
                            theme.success(
                                `Found ${changedFiles.length} changed files in local Git repository`,
                            ),
                        );
                    ...
                    this.context.logger.passThrough(
                        'log',
                        theme.info('Defaulting to current directory.'),
                    );
                    options.path = ['.'];
                }
            }
        ...
        for (const path of changedFiles) {
            ...
            try {
                ...
                if (analysis) {
                    ...
                    this.context.logger.passThrough(
                        'log',
                        theme.success(`✓ Analysis complete for ${path}`),
                    );
                }
            } catch (error) {
            ...
            for await (const entry of Deno.readDir(directory)) {
                ...
                if (entry.isDirectory) {
                    ...
                    if (validExtensions.includes(ext)) {
                        ...
                        this.context.logger.passThrough(
                            'log',
                            theme.dim(`📄 Analyzing file ${entryPath}...`),
                        );
        ...
        try {
            ...
            try {
                ...
                if (perspective === 'all') {
                    ...
                    const juniorAnalysis = await this.analyzeFileWithPerspective(
                        filePath,
                        'junior',
                    );
                    this.context.logger.passThrough(
                    ...
                    const seniorAnalysis = await this.analyzeFileWithPerspective(
                        filePath,
                        'senior',
                    );
                    this.context.logger.passThrough(
                    ...
                    const architectAnalysis = await this.analyzeFileWithPerspective(
                        filePath,
                        'architect',
                    );
                    this.context.logger.passThrough(
                        'log',
                        theme.success('✓ Architect perspective complete'),
                    );

                    // Combine the issues, suggestions and learning opportunities
                    // but add perspective markers to distinguish the source
                    const allIssues = [
                        ...juniorAnalysis.issues.map((issue) => ({
                            ...issue,
                            perspective: 'junior' as const,
                        })),
                        ...seniorAnalysis.issues.map((issue) => ({
                            ...issue,
                            perspective: 'senior' as const,
                        })),
                    ...
                    const avgScore = Math.round(
                        (juniorAnalysis.score + seniorAnalysis.score + architectAnalysis.score) /
                            3 * 10,
                    ) / 10;
                    ...
                    this.context.logger.passThrough(
                        'log',
                        theme.success('✓ Combined analysis complete\n'),
                    );
                } else {
                    ...
                    analysis = await this.analyzeFileWithPerspective(
                        filePath,
...
Respond ONLY with valid JSON - no other text before or after. All severity values must be lowercase ("high", "medium", or "low").`;
        ...
        try {
            ...
            return {
                ...
                learningOpportunities: [
                    `Consider reviewing the file manually due to parsing issues`,
                ],
            };
        }
    }
...
- Cross-cutting concerns`;
    ...
    public async synthesizeReviews(analyses: FileAnalysis[]): Promise<ReviewSynthesis> {
        try {
            ...
            this.context.logger.passThrough(
                'log',
                theme.dim('💭 Generating comprehensive review...'),
            );
      ...
      5. Learning opportunities for the team`;
            ...
            const finalPrompt =
                `Review these file analyses and provide a comprehensive synthesis:\n${
            ...
            try {
                ...
                this.context.logger.passThrough(
                    'log',
                    theme.success('✓ Review synthesis complete'),
                );
            } catch (aiError) {
                this.context.logger.error(
                    `Error in AI synthesis: ${
            ...
            if (
                synthesis.learningOpportunities.length === 0 && allLearningOpportunities.length > 0
            ) {
                synthesis.learningOpportunities = [...new Set(allLearningOpportunities)].slice(
                    0,
                    5,
                );
            }
            ...
            if (!synthesis.actionItems || synthesis.actionItems.length === 0) {
                this.context.logger.passThrough(
                    'log',
                    theme.dim('Generating action items from issues...'),
                );
...
FileAnalyses length: ${session?.overallReview?.fileAnalyses?.length || 0}`);
            ...
            if (synthesis.actionItems && synthesis.actionItems.length > 0) {
                ...
                const highPriority = synthesis.actionItems.filter((item) =>
                    item.priority === 'high'
                );
                const mediumPriority = synthesis.actionItems.filter((item) =>
...
session.mr.changes length: ${session?.mr?.changes?.length}`);
            ...
            if (totalIssues > 0) {
                table.push([
                    `  - 🔴 High: ${highIssues} (${Math.round(highIssues / totalIssues * 100)}%)`,
                ]);
                table.push([
                    `  - 🟠 Medium: ${mediumIssues} (${
                ...
                table.push([
                    `  - 🟡 Low: ${lowIssues} (${Math.round(lowIssues / totalIssues * 100)}%)`,
                ]);
            }
            ...
            if (options?.reviewer) {
                ...
                if (showAllPerspectives) {
                    ...
                    const emoji =
                        perspectiveEmojis[options.reviewer as keyof typeof perspectiveEmojis] || '';
            ...
            if (filesWithIssues > 0) {
                ...
                for (const file of fileAnalyses) {
                    if (file.issues.length > 0) {
                        ...
                        const mediumCount = file.issues.filter((i) =>
                            i.severity === 'medium'
                        ).length;
                ...
                if (highIssues.length > 0) {
                    ...
                    for (const issue of highIssues) {
                        ...
                        const perspEmoji = issue.perspective
                            ? this
                                .perspectiveEmojis[
                                    issue.perspective as keyof typeof this.perspectiveEmojis
                                ] ||
                                ''
                            : '';

                        output.push(
                            `- ${perspEmoji} **${issue.message}**${
    ...
    private async analyzeFileChanges(
        ...
        try {
            ...
            this.context.logger.passThrough(
                'log',
                theme.dim(
                    `Analyzing ${changedSections.length} changed section(s) in ${filePath}...`,
                ),
            );
            ...
            if (analysis.issues) {
                analysis.issues = analysis.issues.map((issue) => {
                    if (issue.line !== undefined) {
                        ...
                        const lineNum = typeof issue.line === 'string'
