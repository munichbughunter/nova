...
export const CodeMetricsSchema = z.object({
    ...
    testCoverage: z.number().min(0).max(100).optional().describe(
        'Test coverage percentage (0-100)',
    ),
    documentation: z.number().min(0).max(100).optional().describe(
        'Documentation quality score (0-100)',
    ),
    bestPractices: z.number().min(0).max(100).optional().describe(
        'Adherence to best practices score (0-100)',
    ),
});
...
export const RecommendationSchema = z.union([
    ...
    z.object({
        ...
        perspective: z.enum(['junior', 'senior', 'architect']).optional().describe(
            'The perspective making this recommendation',
        ),
    }),
...
export const FileAnalysisSchema = z.object({
    ...
    learningOpportunities: z.array(z.string()).describe(
...
export const ReviewSynthesisSchema = z.object({
    ...
    differences: z.array(z.string()).describe(
        'Important disagreements or different emphasis points',
    ),
...
export const MRCommentSchema = z.object({
    ...
    line: z.number().nullable().describe(
        'Line number where the comment should be added, null for general comments',
    ),
...
export const OverallReviewSchema = z.object({
    ...
    fileAnalyses: z.array(FileAnalysisSchema).describe('File analyses included in this review')
        .optional(),
});
