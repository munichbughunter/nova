...
async function getRepoUrl(): Promise<string> {
    try {
        // Try using GitProviderDetector first for better provider support
        const { GitProviderDetector } = await import('../../services/git_provider_detector.ts');
        const repoInfo = await GitProviderDetector.getCurrentRepositoryInfo();

        if (repoInfo && repoInfo.host && repoInfo.projectPath) {
            return `https://${repoInfo.host}/${repoInfo.projectPath}`;
        }
