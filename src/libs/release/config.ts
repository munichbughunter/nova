...
export async function loadReleaseConfig(): Promise<ReleaseConfig> {
  try {
    ...
    throw new Error(
      `Failed to load release config: ${error instanceof Error ? error.message : 'Unknown error'}`,
    );
  }
}
...
export async function checkExistingSemanticRelease(): Promise<boolean> {
  ...
  try {
    ...
    return (
      ...
      (packageJson.devDependencies && packageJson.devDependencies['semantic-release'] !== undefined)
    );
  } catch {
