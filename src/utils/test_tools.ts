...
export class ToolTester {
  ...
  async testTool(
    ...
    this.logger.info(`Testing tool "${toolName}" with params:`, JSON.stringify(params, null, 2));
   ...
   */
  async testSequence(steps: Array<{
    toolName: string;
    params: Record<string, unknown>;
    context?: Partial<MCPToolContext>;
  }>): Promise<MCPToolResult[]> {
    ...
    for (const [index, step] of steps.entries()) {
      this.logger.info(`Running test sequence step ${index + 1}/${steps.length}: ${step.toolName}`);
