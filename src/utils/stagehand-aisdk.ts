...
export class AISdkClient extends LLMClient {
  ...
  private ensureDefaultFields(result: unknown, schema: unknown): unknown {
    if (!result || Object.keys(result).length === 0) {
      ...
      try {
          ...
          for (const key of schema._cached.keys) {
            ...
            if (fieldSchema?._def) {
              ...
              if (defaultValue !== null) {
                ...
                this.log('FIX', `Generated default for '${key}': ${JSON.stringify(defaultValue)}`);
              }
            }
          }
        ...
        this.log('FIX', `✅ Fixed result with ${Object.keys(defaultValues).length} default values`);
        return mergedResult;
      } catch (error) {
  ...
  async createChatCompletion<T = ChatCompletion>({
    ...
    this.log('REQUEST', `Model: ${this.model.modelId}, MessageCount: ${options.messages.length}`);
    ...
    if (this.debug) {
      ...
      if (options.response_model) {
        this.log('REQUEST', `Using response model: ${options.response_model.name || 'unnamed'}`);
      }
      
      // Log tools summary if present
      if (options.tools?.length) {
        this.log('REQUEST', `Using ${options.tools.length} tools`, 
    ...
    if (options.response_model?.name === 'Observation') {
      ...
      try {
        ...
        this.log('RESPONSE', `✅ Observation request #${currentRequestId} completed successfully`);
    ...
    const formattedMessages: CoreMessage[] = options.messages.map((message) => {
      if (Array.isArray(message.content)) {
        this.log('FORMAT', `Converting ${message.role} message with ${message.content.length} content parts`);
    ...
    if (options.response_model) {
      this.log('PROCESS', `🔄 Processing request #${currentRequestId} with object generation`);
      ...
      try {
        ...
        let resultObject = this.ensureDefaultFields(response.object, options.response_model.schema);
  
        // Handle specific schemas for metadata responses
        if (options.response_model.name === 'Metadata' && 
           (!resultObject.progress || !Object.hasOwnProperty.call(resultObject, 'completed'))) {
        ...
        if (options.response_model.name === 'Extraction' || 
  ...
  private parseObservationResponse(response: string): Array<{ description: string; selector: string }> {
    try {
